...
function setBoard() {
  ...
  function generateBoggleGame() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      for (let j = 0; j < 4; j++) {
        //creates columns/elements
        let randomLetter =
          gameAlphabet[Math.floor(Math.random() * Math.floor(26))]; //Generates random index to pull from alphabet
...
function resetValid() {
  ...
  while (counter < validStartValue.length) {
    document.getElementById(`${validStartValue[counter]}`).title = "invalid";
    ...
    if (rowCount[r] > -1 && rowCount[r] < 4) {
      for (let c = 0; c < 4; c++) {
        if (colCount[c] > -1 && colCount[c] < 4) {
          document.getElementById(`cell-${rowCount[r]}-${colCount[c]}`).title =
            "validOption";
          valid.push(
            document.getElementById(`cell-${rowCount[r]}-${colCount[c]}`).id
          );
        }
      }
    }
